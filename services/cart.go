...
type cartRepository interface {
	...
	AddVasItemToItemByItemID(itemId primitive.ObjectID, vasItem *domain.VasItem) error
}
...
func NewCartService(cartRepository cartRepository, promotionServiceClient promotionServiceClient) *cartService {
	return &cartService{
		cartRepository:         cartRepository,
		promotionServiceClient: promotionServiceClient,
	}
...
func (s *cartService) addItemToCartRuleChain(ctx context.Context, cart *domain.Cart) bool {
	...
	for _, rule := range rules.GetRulers() {
...
func (s *cartService) addVasItemToItemRuleChain(ctx context.Context, item *domain.Item, totalPrice float64) bool {
	...
	for _, rule := range rules.GetVasRulers() {
