...
func (s *FileServer) read() ([]string, error) {
	...
	f, err := os.Open(config.Config.Server.FileServer.InputPath)
	if err != nil {
...
func (s *FileServer) writeSucceedResponse(ctx context.Context, input string) error {
	...
	f, err := os.OpenFile(config.Config.Server.FileServer.OutputPath, os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		l.Errorw(
			fmt.Sprintf("file open error: %v", err),
			"outputPath", config.Config.Server.FileServer.OutputPath,
		)
		return err
	}
	defer f.Close()
